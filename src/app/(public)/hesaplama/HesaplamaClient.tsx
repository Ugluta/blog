...
function Row({ label, value, strong, color }: { label: string; value: string; strong?: boolean; color?: string }) {
  return (
    <div className={`flex items-center justify-between py-2 ${strong ? 'border-t border-gray-200 mt-1 pt-2.5' : 'border-b border-gray-50'}`}>
    ...
    <div>
      ...
      <div className="flex">
        <input
          ...
          placeholder="0"
          className="flex-1 h-10 px-3 text-sm border border-gray-200 rounded-l-lg focus:outline-none focus:ring-2 focus:ring-blue-500 min-w-0"
        />
        {allowUnit && unit && onUnit ? (
          <div className="flex border border-l-0 border-gray-200 rounded-r-lg overflow-hidden">
            <button type="button" onClick={() => onUnit('percent')} className={`px-2.5 text-sm font-medium ${unit === 'percent' ? 'bg-blue-600 text-white' : 'bg-gray-50 text-gray-500'}`}>%</button>
            <button type="button" onClick={() => onUnit('amount')} className={`px-2.5 text-sm font-medium ${unit === 'amount' ? 'bg-blue-600 text-white' : 'bg-gray-50 text-gray-500'}`}>₺</button>
          </div>
        ) : (
          <span className="inline-flex items-center px-3 text-sm text-gray-500 bg-gray-50 border border-l-0 border-gray-200 rounded-r-lg">{suffix ?? '₺'}</span>
        )}
      </div>
    </div>
  );
    ...
    <div className="grid lg:grid-cols-2 gap-6">
      ...
      <div>
        <div className="flex gap-2 p-1 bg-gray-100 rounded-xl mb-4">
          <button onClick={() => setMode('kar')} className={`flex-1 flex items-center justify-center gap-2 h-10 rounded-lg text-sm font-semibold transition-colors ${mode === 'kar' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500'}`}>
            <TrendingUp className="w-4 h-4" /> Kâr Hesapla
          </button>
          <button onClick={() => setMode('satis')} className={`flex-1 flex items-center justify-center gap-2 h-10 rounded-lg text-sm font-semibold transition-colors ${mode === 'satis' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500'}`}>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-5">
          ...
          <p className="flex items-start gap-2 text-xs text-gray-400 mt-3">
            ...
            Komisyon ve ekstra “%” seçiliyse <strong className="font-semibold text-gray-500">satış fiyatı</strong> üzerinden hesaplanıp satıştan düşülür.
          </p>
        </div>
      </div>
      ...
      <div className="lg:sticky lg:top-24 self-start">
        {mode === 'kar' ? (
          <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
            <div className={`p-5 text-center ${netKar >= 0 ? 'bg-gradient-to-br from-emerald-500 to-teal-600' : 'bg-gradient-to-br from-rose-500 to-red-600'} text-white`}>
          ...
          <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
            <div className="p-5 text-center bg-gradient-to-br from-blue-600 to-violet-600 text-white">
              ...
              <p className="text-sm text-white/80 mt-1">{P > 0 ? `${tl(P)} net kâr için` : ''}</p>
            </div>
            <div className="p-5">
              {gecersiz ? (
