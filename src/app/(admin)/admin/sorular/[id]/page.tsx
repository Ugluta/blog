...
export default function SoruDuzenlePage() {
  ...
  const [form, setForm] = useState({
    ...
    tags: '', isPublic: true, isApproved: false,
  });
  ...
  useEffect(() => {
    Promise.all([
      fetch(`/api/sorular/${id}`).then((r) => r.json()),
      fetch('/api/kategoriler?type=subjects').then((r) => r.json()),
    ]).then(([q, subs]) => {
      if (q) {
        setForm({
          content: q.content || '',
  ...
  const handleSave = async () => {
    ...
    const res = await fetch(`/api/sorular/${id}`, {
      method: 'PATCH',
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-xl font-bold text-gray-900">Soru Düzenle</h1>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-6 space-y-5">
        ...
        {(form.type === 'MULTIPLE_CHOICE' || form.type === 'TRUE_FALSE') && (
          <div>
            ...
            <div className="space-y-2">
              {options.map((opt, i) => (
                <div key={i} className="flex items-center gap-2">
                  ...
                  <input value={opt.text} onChange={(e) => setOptions(options.map((o, idx) => idx === i ? { ...o, text: e.target.value } : o))}
            ...
            <p className="text-xs text-gray-400 mt-1">Doğru cevap için şık harfine tıklayın (yeşil = doğru)</p>
          </div>
        )}
        ...
        <div>
          ...
          <input value={form.answer} onChange={(e) => setForm({ ...form, answer: e.target.value })}
            className="w-full h-10 px-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
            placeholder="Kısa cevap veya doğru şık" />
        </div>
