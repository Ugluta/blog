...
export async function POST(req: Request) {
  ...
  const { id, title, description, features, price, icon, isActive, sortOrder } = b;
  ...
  const created = await db.service.create({ data: { ...data, slug: `${toSlug(title)}-${Date.now().toString(36)}` } });
